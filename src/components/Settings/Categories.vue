<template>
    <section class="p-8 w-full">
        <div class="section-hedear flex">
            ...
            <h2 class="ml-auto text-3xl sm:text-xl text-site-text-primary dark:text-white mb-5">انواع البلاغات</h2>
        </div>
        <div class="card" style="direction: rtl;" v-if="useUserStore().userPermissions.show_Category">
        <DataTable :value="categories" dataKey="id" paginator :rows="5" 
                >
            <template #empty>لم يتم العثور على اي نوع </template>
            <Column field="category"  header="نوع البلاغ"  style="min-width: 12rem;" class="text-center text-site-primary dark:text-white" headerClass="w-6" :pt="{ headerContent: 'justify-center gap-1'}">
                <template #body="{ data }" >
            ...
            <Column :exportable="false" style="min-width:10rem">
                <template #body="slotProps">
                    ...
                    <Button icon="pi pi-trash" v-if="useUserStore().userPermissions.delete_Category" outlined rounded severity="danger" @click="confirmDeleteRequest($event,slotProps.data)" />
                </template>
            </Column>
        </DataTable>
    </div>
    <hr>
    <div class="card my-7" style="direction: rtl;" v-if="useUserStore().userPermissions.create_Category">
        <h2 class="ml-auto text-3xl sm:text-xl text-site-text-primary dark:text-white mb-5">اضافة نوع بلاغ</h2>
                ...
                <div>
                    <div class="border-b-2 border-site-primary ">
                        <FloatLabel>
                            ...
                            <label for="name" class=" text-site-primary" > نوع البلاغ</label>
                        </FloatLabel>
                    </div> 
...
onMounted(async() => {
    const { Data } = await useGetRequest('Category')
    categories.value = Data.value;
    loading.value = false;
});
const submit = async function(){
            isLoading.value = true
                const { Error } = await usePostRequest('Category',{'name':name.value})
                requestError.value = Error.value
                isLoading.value = false
            if(!requestError.value){
                notify({
                    title: "نوع البلاغ",
                    text: "تم انشاء نوع البلاغ بنجاح",
                    type: 'success',
                });
...
const confirmDeleteRequest = (event,data) => {
    confirm.require({
        ...
        acceptLabel: 'حذف',
        accept: async () => {
          await useDeleteRequest('Category/'+data.id)
          router.go('')      
        },
        reject: () => {

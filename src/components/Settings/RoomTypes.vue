<template>
    <section class="p-8 w-full">
        <div class="section-hedear flex">
            ...
            <h2 class="ml-auto text-3xl sm:text-xl text-site-text-primary dark:text-white mb-5">انواع الغرف</h2>
        </div>
        <div class="card" style="direction: rtl;">
        <DataTable :value="roomtypes" dataKey="id" paginator :rows="5" 
                >
            <template #empty>لم يتم العثور على اي انواع غرف</template>
            <Column field="building"  header="نوع الغرفة"  style="min-width: 12rem;" class="text-center text-site-primary dark:text-white" headerClass="w-6" :pt="{ headerContent: 'justify-center gap-1'}">
                <template #body="{ data }" >
    ...
    <hr>
    <div class="card my-7" style="direction: rtl;">
        <h2 class="ml-auto text-3xl sm:text-xl text-site-text-primary dark:text-white mb-5">اضافة نوع غرفة</h2>
                ...
                <div>
                    <div class="border-b-2 border-site-primary ">
                        <FloatLabel>
                            ...
                            <label for="name" class=" text-site-primary" > نوع الغرفة</label>
                        </FloatLabel>
                    </div> 
...
onMounted(async() => {
    const { Data } = await useGetRequest('RoomType')
    roomtypes.value = Data.value;
    loading.value = false;
});
const submit = async function(){
            isLoading.value = true
                const { Error } = await usePostRequest('RoomType',{'name':name.value})
                requestError.value = Error.value
                isLoading.value = false
            if(!requestError.value){
                notify({
                    title: "نوع الغرفة",
                    text: "تم انشاء نوع الغرفة بنجاح",
                    type: 'success',
                });
...
const confirmDeleteRequest = (event,data) => {
    confirm.require({
        ...
        acceptLabel: 'حذف',
        accept: async () => {
          await useDeleteRequest('RoomType/'+data.id)
          router.go('')      
        },
        reject: () => {
